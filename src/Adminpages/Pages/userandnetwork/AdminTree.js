...
const mergeChildren = (node, targetId, newChildren) => {
  ...
  return {
    ...node,
    children: node.children.map((c) => mergeChildren(c, targetId, newChildren)),
  };
};
...
const setLoadingState = (node, targetId, loading) => {
  ...
  return {
    ...node,
    children: node.children.map((c) => setLoadingState(c, targetId, loading)),
  };
};
...
const AdminTree = () => {
  ...
  const loadChildren = useCallback(async (nodeDatum) => {
    ...
    if (!reg_id || isNaN(reg_id)) {
      console.error(
        "Cannot expand: reg_id (lgn_jnr_id) is missing for node",
        mem_id,
    ...
    try {
      const res = await apiConnectorGetAdmin(
        endpoint.get_member_downline_tree,
        {
  ...
  const activeStatus =
    selectedNode?._topupDate && selectedNode._topupDate !== "0";
  const popupRows = [
    ...
    return (
      ...
      >
        ...
        <p className="text-gray-600 text-sm">
          This user's team tree will appear here once members join.
        </p>
      </div>
    );
    ...
    <>
      ...
      <Menu
        ...
        PaperProps={{
          style: {
            background:
              "linear-gradient(135deg, #060d1a 0%, #080f1e 50%, #060a14 100%)",
            border: "1px solid rgba(34,211,238,0.25)",
            borderRadius: "14px",
            boxShadow:
              "0 8px 32px rgba(34,211,238,0.12), 0 2px 8px rgba(0,0,0,0.6)",
            padding: "4px",
            minWidth: "270px",
          },
        }}
          ...
          <div className="w-full" onClick={(e) => e.stopPropagation()}>
            ...
            {popupRows.map((row, idx) => (
              <div
                ...
                style={{
                  background:
                    idx % 2 === 0 ? "rgba(34,211,238,0.04)" : "transparent",
