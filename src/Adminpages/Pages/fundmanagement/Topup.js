  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg, rgba(10,18,25,0.99) 0%, rgba(13,24,33,0.97) 100%)",
          border: "1px solid rgba(34,211,238,0.2)",
          boxShadow:
            "0 20px 60px rgba(0,0,0,0.7), 0 0 60px rgba(34,211,238,0.06)",
        }}
      >
        ...
        <div className="relative z-10 px-8 py-8 flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            ...
            >
              {mode === "totp" ? "Authenticator Verification" : "Email Verification"}
            </h3>
            <p className="text-xs text-gray-500 leading-relaxed">
              {mode === "totp" ? (
                <>Enter the 6-digit code from your<br />authenticator app to confirm this TopUp</>
              ) : (
                <>Enter the 6-digit code sent to your<br />email to confirm this TopUp</>
              )}
            </p>
          </div>
...
const TopUp = () => {
  ...
  const [otpStep, setOtpStep] = useState(null); // null | "mail" | "totp"
  ...
  const areYouSureFn = async () => {
    ...
    if (!topup_amnt) {
      Swal.fire({
        text: "Please enter a valid TopUp amount",
        confirmButtonColor: "black",
        icon: "warning",
      });
      return;
    }
    if (!fk.values.topup_desc.trim()) {
      Swal.fire({
  ...
  // ── Step 2: Send mail OTP → open mail OTP modal ─────────────────────────────
  const sendTopupOTP = async () => {
    try {
      setLoading(true);
      const res = await apiConnectorPostAdmin(endpoint?.send_otp_topup, {});
      if (res?.data?.success) {
        setOtpStep("mail");
      } else {
        Swal.fire({
      ...
      Swal.fire({
        ...
        title: "Failed to send OTP",
        text:
          err.response?.data?.msg || "An error occurred while sending OTP.",
      });
    } finally {
      setLoading(false);
    }
  ...
  // ── Step 3/4: Verify mail OTP → switch to authenticator OTP → proceed to TopUp
    ...
    try {
      if (otpStep === "mail") {
        ...
        if (verifyRes?.data?.success) {
          ...
          setOtpError(
            verifyRes?.data?.msg || "Invalid code. Please try again.",
          );
        }
        return;
      }

      const verifyRes = await apiConnectorPostAdmin(endpoint?.verify_totp, {
        otp: otpCode,
      });
      ...
      if (verifyRes?.data?.success) {
        ...
        TopUpFn({
          ...
          desc: fk.values.topup_desc,
        });
      } else {
        setOtpError(verifyRes?.data?.msg || "Invalid code. Please try again.");
      }
    } catch (err) {
      setOtpError(err.response?.data?.msg || err.message);
    } finally {
    ...
    try {
      ...
      setSponsername(
        response?.data?.success
          ? response?.data?.result?.[0]?.lgn_name
          : "Invalid Sponser",
      );
    } catch {
    ...
    <>
      ...
      <div className="flex justify-center items-start w-full py-4">
        ...
        <div
          ...
          style={{
            background:
              "linear-gradient(135deg, rgba(10,18,25,0.97) 0%, rgba(13,24,33,0.95) 100%)",
            border: "1px solid rgba(34,211,238,0.15)",
            boxShadow:
              "0 8px 40px rgba(0,0,0,0.5), 0 0 40px rgba(34,211,238,0.04)",
          }}
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2
                  ...
                  style={{
                    background:
                      "linear-gradient(90deg, #cffafe, #67e8f9, #38bdf8)",
                    WebkitBackgroundClip: "text",
                    WebkitTextFillColor: "transparent",
                  }}
                >
          ...
          <div className="relative z-10 px-7 py-6 flex flex-col gap-5">
            ...
            <div className="relative">
              <label
                className="absolute -top-2 left-3 text-[9px] font-bold tracking-widest uppercase px-1.5 py-0.5 rounded z-10"
                style={{
              ...
              <div className="relative">
                ...
                />
                {sponsername && (
                  <span className="!text-red-500 !text-[10px] mt-1 block pl-1">
                    {sponsername}
                  </span>
                )}
              </div>
          ...
          >
            ...
            <button
              onClick={hasPermission("fund.topup") ? areYouSureFn : undefined}
              type="button"
              ...
              style={{
                background:
                  "linear-gradient(135deg, rgba(6,182,212,0.22) 0%, rgba(14,116,144,0.16) 100%)",
                border: "1px solid rgba(34,211,238,0.35)",
                color: "rgba(34,211,238,0.95)",
                boxShadow: "0 0 16px rgba(34,211,238,0.1)",
              }}
              onMouseEnter={(e) => {
                if (hasPermission("fund.topup")) {
                  e.currentTarget.style.background =
                    "linear-gradient(135deg, rgba(6,182,212,0.32) 0%, rgba(14,116,144,0.24) 100%)";
          ...
          <div
            className="absolute top-16 right-8 w-1 h-1 bg-cyan-400 rounded-full opacity-40 animate-ping pointer-events-none"
            style={{ animationDuration: "3s" }}
          />
          <div
            className="absolute bottom-20 left-10 w-1 h-1 bg-blue-400 rounded-full opacity-30 animate-ping pointer-events-none"
