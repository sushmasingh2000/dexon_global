  ...
  async function generateDepositQrCode() {
    ...
    try {
      ...
      const qrCode = await apiConnectorPost(endpoint?.get_topup_qr, {
        user_amount: fk.values.inr_value,
      });

      if (qrCode?.data?.success) {
        const qrData = qrCode?.data?.result?.[0];
        setQrCode(qrData);
        setTimeLeft(300);
        setTimerActive(true);
      } else {
          ...
          <div className="relative z-10">
            ...
            {qrCode && (
              <div className=" flex flex-col items-center justify-center">
                <img src={qrCode?.qr || qrCode?.qrcode_url} alt="QR Code" className="w-32 h-32" />
...
export default CryptoFitGateway;
