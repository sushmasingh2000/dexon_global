        ...
        <form onSubmit={fk.handleSubmit}>
          <div className="px-6 py-5 space-y-4">
            ...
            <div>
              <p className="text-[9px] text-cyan-400/50 uppercase tracking-widest font-bold mb-3 flex items-center gap-2">
                ...
                Change Password <span className="text-gray-600 normal-case">(optional)</span>
                <span className="flex-1 h-px bg-cyan-400/10" />
              </p>
              <div className="space-y-3">
                <ModalInput label="New Password" name="password"
                  type={showPass ? "text" : "password"} value={fk.values.password}
                  onChange={fk.handleChange} placeholder="Leave blank to keep current"
                  rightSlot={<EyeToggle visible={showPass} onToggle={() => setShowPass(p => !p)} />} />
                <ModalInput label="Confirm Password" name="confirmPassword"
                  type={showConfirm ? "text" : "password"} value={fk.values.confirmPassword}
                  onChange={fk.handleChange} placeholder="Repeat new password"
          ...
          <div className="flex gap-3 px-6 pb-5" style={{ borderTop: "1px solid rgba(34,211,238,0.06)", paddingTop: "1rem" }}>
            ...
            <button type="submit" disabled={loading}
              className="flex-1 py-2.5 rounded-xl text-xs font-bold overflow-hidden relative group/btn transition-all duration-300 hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
              style={{ background: "linear-gradient(135deg, rgba(6,182,212,0.25) 0%, rgba(14,116,144,0.18) 100%)", border: "1px solid rgba(34,211,238,0.35)", color: "rgba(34,211,238,0.95)" }}>
...
const MemberList = () => {
  ...
  const copyWalletAddress = async (walletAddress) => {
    ...
    try {
      ...
      Swal.fire({ title: "Copy Failed", text: "Unable to copy wallet address.", icon: "error" });
    }
  };
  ...
  const tablerow = allData?.data?.map((row, index) => {
    ...
    return [
      ...
      // ── Edit button ──
      <button title="Edit Profile" onClick={() => setEditRow(row)}
