    ...
    {visible ? (
      ...
      >
        <path
          ...
          strokeWidth={2}
          d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.956 9.956 0 012.042-3.368m2.087-1.858A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a9.97 9.97 0 01-4.161 5.065M15 12a3 3 0 11-6 0 3 3 0 016 0zm6 9L3 3"
        />
      </svg>
    ) : (
      <svg
  ...
  const fk = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const result = await Swal.fire({
        title:
          '<span style="color:#22d3ee;font-size:1rem;">Confirm Update?</span>',
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "linear-gradient(135deg,rgba(10,18,25,0.97),rgba(13,24,33,0.95))",
      ...
      >
        ...
        <form onSubmit={fk.handleSubmit}>
          <div className="px-6 py-5 space-y-4">
            ...
            {hasPermission("members.update_password") && (
              <div>
                <p className="text-[9px] text-cyan-400/50 uppercase tracking-widest font-bold mb-3 flex items-center gap-2">
                  <span className="flex-1 h-px bg-cyan-400/10" />
                  Change Password{" "}
          ...
          >
            ...
            <button
              ...
              style={{
                background:
                  "linear-gradient(135deg,rgba(6,182,212,0.25),rgba(14,116,144,0.18))",
...
const SubAdminToggle = ({ enabled, onChange, loading }) => (
  <button
    ...
    className={`relative inline-flex items-center w-12 h-6 rounded-full transition-all duration-300 focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed
      ${enabled ? "shadow-[0_0_10px_rgba(168,85,247,0.5)]" : "shadow-none"}`}
    style={{
      background: enabled
...
const SubAdmin2FAToggle = ({ enabled, onChange, loading }) => (
  <button
    onClick={onChange}
    disabled={loading}
  ...
  >
    <span
      className={`absolute top-0.5 w-5 h-5 rounded-full transition-all duration-300 shadow-md ${enabled ? "left-[26px] bg-white" : "left-0.5 bg-gray-500"}`}
    />
  </button>
);
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await apiConnectorPostAdmin(
          endpoint?.update_member_max_payout,
          {
...
const MemberList = () => {
  ...
  const [loading, setLoading] = useState(false);
  const [subAdminLoadingId, setSubAdminLoadingId] = useState(null);
  const [twoFaLoadingId, setTwoFaLoadingId] = useState(null);
  const [twoFaOverrides, setTwoFaOverrides] = useState({});
  ...
  const isSubAdminUser = localStorage.getItem("user_type") === "SubAdmin";
    ...
    const result = await Swal.fire({
      ...
      html: `<p style="color:#94a3b8;font-size:0.85rem;">${
        isCurrentlyBlocked
          ...
          : `Prevent <strong style="color:#ef4444">${row?.lgn_name}</strong> from logging in?`
      }</p>`,
  ...
  async function toggleWithdrawal(row, walletType) {
    ...
    try {
      const res = await apiConnectorPostAdmin(
        endpoint?.member_withdrawal_permission,
        {
      ...
      if (String(res?.data?.success) === "true") {
        ...
        toast.error(
          res?.data?.message || "Failed to update withdrawal permission.",
        );
      }
    } catch (e) {
      console.error(e);
  ...
  async function toggleTrade(row) {
    ...
    try {
      const res = await apiConnectorPostAdmin(
        endpoint?.member_trade_permission,
        {
          customer_id: row?.tr03_reg_id,
          status: isAllowed ? 0 : 1,
        },
  ...
  async function toggleSubAdminAccess(row) {
    ...
    try {
      const res = await apiConnectorPostAdmin(
        endpoint?.toggle_subadmin_access,
        {
      ...
      if (String(res?.data?.success) === "true") {
        toast.success(
          res?.data?.message || `SubAdmin access ${action.toLowerCase()}ed.`,
        );
        client.invalidateQueries(["get_member_list"]);
        ...
        if (!isCurrentlySubAdmin) {
          const goNow = await Swal.fire({
            title:
              '<span style="color:#a855f7;font-size:1rem;">Access Granted!</span>',
            html: `<p style="color:#94a3b8;font-size:0.85rem;">
            ...
            cancelButtonColor: "#374151",
            confirmButtonText: "Set Permissions →",
  ...
  // ── SubAdmin 2FA Toggle ────────────────────────────────────────────────────
  async function toggleSubAdmin2FA(row) {
    const loginId = row?.tr03_lgn_id;
    setTwoFaLoadingId(loginId);
    try {
      const token =
        localStorage.getItem("logindataen_admin") ||
        localStorage.getItem("token");
      const res = await apiConnectorPostAdmin(endpoint.toggle_subadmin_2fa, {
        login_id: loginId,
      });
      if (res.status === 200) {
        const record = res.data?.data?.[0];
        setTwoFaOverrides((prev) => ({
          ...prev,
          [loginId]: Number(record?.lgn_is_2fa_scanned),
        }));
        toast.success(res.data?.message || "2FA status updated.");
      } else {
        toast.error(res.data?.message || "Failed to toggle 2FA.");
      }
    } catch (e) {
      toast.error(
  ...
  const tablerow = allData?.data?.map((row, index) => {
    ...
    const twoFaStatus = Number(row?.lgn_is_2fa_scanned);
    const isTwoFaLoading = twoFaLoadingId === row?.tr03_lgn_id;
    ...
    return [
      ...
      // Edit
      hasPermission("members.update_profile") ? (
        <button
          title="Edit Profile"
      ...
      hasPermission("members.update_max_payout") ? (
        <span
      ...
      // Withdrawal
      hasPermission("members.withdrawal_toggle") ? (
        <Switch
          checked={Number(row?.tr03_active_for_growth_payout) === 1}
          onChange={() => toggleWithdrawal(row, "Growth")}
          color="success"
        />
      ) : (
        <Switch
      ...
      hasPermission("members.withdrawal_toggle") ? (
        <Switch
          checked={Number(row?.tr03_active_for_earning_payout) === 1}
          onChange={() => toggleWithdrawal(row, "Earning")}
          color="success"
        />
      ) : (
        <Switch
      ...
      // Trade
      hasPermission("members.trade_toggle") ? (
        <Switch
      ...
      ...(!isSubAdminUser
        ? [
            ...
            isSubAdmin ? (
              <div className="flex flex-col items-center gap-1">
                <SubAdmin2FAToggle
                  enabled={twoFaStatus === 1}
                  onChange={() => toggleSubAdmin2FA(row)}
                  loading={isTwoFaLoading}
                />
                <span
                  className={`text-[9px] font-mono font-bold ${twoFaStatus === 1 ? "text-green-400" : "text-gray-500"}`}
                >
                  {twoFaStatus === 1 ? "1 · ON" : "0 · OFF"}
                </span>
              </div>
            ) : (
              <span className="text-gray-700 text-xs">—</span>
            ...
            // ── Block / Unblock ────────────────────────────────────────────────────
            hasPermission("members.block_unblock") ? (
              <button
